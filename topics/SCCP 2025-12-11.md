...
あとは、AtCoder上やAOJ上のコードテスト機能、Paiza ioといったオンライン実行環境ではインタラクティブ形式の実行ができないことも大きな相違点かもしれません。
ターミナル上で実行しないとデバッグが厳しいので、ローカルで実行できる環境を整えたほうがやりやすくなります。
...
ここからは競プロ経験者向け枠の問題なので、一気に難易度が上がります。

また、下の問題3問は結局すべてDPで解けます。若干変則的なアプローチを要するDPを集めました。
解説も雑になります (すいません)
...
>$N$ 頂点の完全グラフから $M$ 本の辺が欠けたようなグラフを考える。
このグラフ上で、頂点1から始めて頂点1で終わるような長さKのパスの総数を mod 998244353 で数え上げせよ
 
$i$ 日目に頂点$j$ にいる状態の通り数をdpで計算すれば答えが求まりますが、計算量が $O(KN^2)$ で制約上間に合いません。
 
グラフの構造に注目すると、次のように言い換えられます:
完全グラフだと仮定したら、寄与は前日分の総和になる。そこから欠けている辺の寄与だけ引いても解は等しい。

よって言い換え後の形で処理すれば $O(K(N+M))$ が達成できる。
...
int main(){
    ...
    int n,m,K;
    cin>>n>>m>>K;
    vector g(n,vector<unsigned int>());
    rep(i,0,m){
        int u,v;
        cin>>u>>v;
        u--,v--;
...
通らなかったチェックポイントに応じて課せられるペナルティに注目します。
通らなかった数を $C$ として $2^{C}$ がペナルティになりますが、つまり $C=30$ くらいの時点でペナルティが激重になることがわかります。

少なくとも総移動距離を上回るペナルティを食らうと本末転倒なので、無視するチェックポイントの個数は高々30個程度としてDPをするとよいです (今の頂点、無視した個数を状態に持つ)。
...
int main() {
    ...
    int C = 35;
    vector dp(n,vector<double>(C,DBL_MAX));
    dp[0][0] = 0.0;
    rep(i,0,n){
        rep(j,0,C){
            ...
            rep(k,1,C){
                if(i+k < n && j+k-1 < C){
                    chmin(dp[i+k][j+k-1], dp[i][j] + calc_dist(i,i+k));
                }
            }
        }
    }
...
```
昔に書いたコードそのままなので汚いしこのまま貼るとテンプレ不足でCEになると思います　すいません
...
食べる順番を自由に決められる点が難しく感じられるかもしれませんが、実は順番は殆ど関係ないです。
結局、今までの摂取甘さ総和が $X$ 以下、しょっぱさ総和が $Y$ 以下の取り方であって個数が最大になるもの(=$cnt$)を求められれば、そこから残ってるもう一品を食ってギブアップになり終了するので、$cnt+1$が答えになります。ただしN品を超えて食べることはできないので(全部食べても$X$ 以下 $Y$ 以下になる場合)、$\min(cnt+1, N)$ が解になります。

ということで、今までの摂取甘さ総和が $X$ 以下、しょっぱさ総和が $Y$ 以下であって最適な取り方(の取った品目の個数) を求められれば本問に答えることができます。
これはなんとなくDPの匂いがしますよね(?)。しかしそのまま甘さ総和としょっぱさ総和を持ちながらDPすると $O(NXY)$ となり、本問の制約上ではACを得るのは厳しいです。

ここで、典型テクの一種ともいえる、添え字の情報と値の情報の入れ替えを考えます。
具体的には、
$dp[i][j][k] := i番目の料理を見ていて、甘さ総和がj、しょっぱさ総和がkのときに、実現できる「食べた料理の個数」の最大値$
を、
$dp[i][j][k] := i番目の料理を見ていて、今までj個の料理を食べていて、甘さ総和がkのとき、実現できるしょっぱさ総和の最小値$
とします(甘さとしょっぱさは順不同)。

後者のDPは $O(N^2X)$ で実行できて、ACを得られる程度に高速です。
    ...
    for(int i=0;i<N;i++){
        ...
        for(int j=0;j<N;j++){
            for(int k=0;k<=X;k++){
                ...
                if(k+A[i]<=X) chmin(ndp[j+1][k+A[i]], dp[j][k]+B[i]);
            }
        }
        dp=std::move(ndp);
    }
